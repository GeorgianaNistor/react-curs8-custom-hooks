...
export default function App() {
	...
	return (
		<div className="App">
			...
			<div className="favourite-user">
				<h3>User favorit</h3>
				{/* Afisam userul favorit. */}
				{favoriteUser ? (
					<div>
						<p>Nume: {favoriteUser.name}</p>
						<p>Email: {favoriteUser.email}</p>
					</div>
				) : (
				...
				users.map((user) => {
					return (
						<div>
							<h2>{user.name}</h2>
							<p>{user.email}</p>
							<button onClick={() => handleUserClick(user.name, user.email)}>
								Selecteaza ca Favorit
							</button>
						</div>
					);
				})}
